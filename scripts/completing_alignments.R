...
### list file names
loci_names = list.files(path = paste0(dtype,"1_aligned_sequences"), 
                        pattern = ".FNA")
...
for(i in 1:length(loci_names)){
  ...
  tryCatch({
    ...
    one_locus = read.fasta(paste0(dtype,"1_aligned_sequences/", locus_names))  
    ### update n_spp
