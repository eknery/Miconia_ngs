...
### list file names
loci_names = list.files(path = "1_aligned_sequences", pattern = ".FNA")
  ...
  tryCatch(
    {
      ...
      one_locus = read.fasta(paste0("1_aligned_sequences/", loci_names[i]))  
      ### update n_spp
      ...
      write.fasta(
        ...
        as.string = F, 
        names = spp_names,
        file.out = paste0(dir_out, loci_names[i]),
        nbchar = 100
      )
    },
    error = function(e) {
...
write.table(n_spp,"n_spp.txt", row.names = F)
